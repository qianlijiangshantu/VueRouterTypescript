    ...
    <div>
      <div>
        ...
        <b-form-group class="col-8" label="NAPT用グローバルIP">
          ...
          <div v-for="item in Napt" :key="item">
            <span class="content pl-3"
              >{{ item }}
              <span v-if="Napt.length > 1" class="ml-3">
                <b-icon-dash-square
                  class="remoteIcon"
        ...
        <b-form-group class="col-8" label="NAT用グローバルIP">
          ...
          <span class="content">払出済グローバルIP</span><br />
          <span class="content pl-3">グローバルIP</span><br />
          <div v-for="item in Nat" :key="item">
            <span class="content pl-3"
              >{{ item }}
              <span v-if="Nat.length > 1" class="ml-3">
                <b-icon-dash-square
                  class="remoteIcon"
...
export default class Home extends Vue {
  ...
  async mounted() {
    ...
    this.Napt = GlobalIpNaptNat.NAPT;
    this.Nat = GlobalIpNaptNat.NAT;
    // グローバルIP数
  ...
  removeNapt(target: string) {
    this.Napt = this.Napt.filter((item: string) => item !== target);
    this.delNapt.push(target);
  }
  removeNat(target: any) {
    this.Nat = this.Nat.filter((item: string) => item !== target);
  ...
  update() {
    ...
    // NaptIP
    this.ipNaptBefore.forEach((item: string) => {
    ...
    // NatIP
    this.ipNatBefore.forEach((item: string) => {
